...
function Start()
{
	playerGUI = GameObject.Find("muneco5").GetComponent(PlayerGUI);	
}
...
function Update()
{
	...
	{
		Time.timeScale = 0;
		GameObject.Find("muneco5").GetComponent(FPSInputController).enabled = false;
		GameObject.Find("muneco5").GetComponent(MouseLook).enabled = false;
		...
		GameObject.Find("Camera").GetComponent(RayCastCollect).enabled = false;
	}
	
	if(showGUI == false)
	{
		Time.timeScale = 1;
		GameObject.Find("muneco5").GetComponent(FPSInputController).enabled = true;
		GameObject.Find("muneco5").GetComponent(MouseLook).enabled = true;
		...
		GameObject.Find("Camera").GetComponent(RayCastCollect).enabled = true;
	}
}
...
function Eat()
{
	playerGUI.hungerBarDisplay += 0.25;
}

function Drink()
{
	playerGUI.thirstBarDisplay += 0.20;
}

function Heal()
{
	playerGUI.healthBarDisplay += 0.25;
}
